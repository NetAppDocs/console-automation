...
summary: The document describes the grant types and identity verification while using NetApp Console APIs
---

= Grant types and identity verification for the NetApp Console REST APIs
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
There are several OAuth 2.0 grant types available when requesting an access token to use with the NetApp Console. The grant type is based on the application requirements and authentication environment, and determines how the token is requested.
...
All grant types use the following authentication endpoint to request an access token:
...
|`client_id`
|The client ID representing the Console. This value is constant and the same for all customers.
|===
...
== Client credentials using a NetApp Console service account

In some cases, static clients or applications need to acquire an access token to perform operations on behalf of a user. Applications need to obtain an access token for their own service account without user intervention. In these situations you can use the client credentials grant type. To use client credentials, you must create a service account to obtain the client ID and secret using the link:https://console.netapp.com/identity-access-mgmt/users#add-user[NetApp Console UI^].

The following is a JSON input example for requesting the token with a description of each parameter.
