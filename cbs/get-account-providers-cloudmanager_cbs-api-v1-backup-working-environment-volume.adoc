---
sidebar: sidebar
api: true
explorer: false
permalink: 'cbs/get-account-providers-cloudmanager_cbs-api-v1-backup-working-environment-volume.html'
summary: 'Retrieve volume details'
---

= Retrieve volume details
:imagesdir: ../media/

[.api-doc-operation .api-doc-operation-get]#GET# [.api-doc-code-block]#`/account/{accountId}/providers/cloudmanager_cbs/api/v1/backup/working-environment/{workingEnvironmentId}/volume`#



Retrieves details of all the volumes for a specified system backed up by using the NetApp Backup and Recovery service. It retrieves the volume name, volume type, protocol, storage VM, backup status, last backup creation time, snapshot count, associated backup policy, and backup policy rules. It also retrieves details of the archive after days, object lock, ransomware scan status, last ransomware scan, and ransomware detected snapshots with last ransomware scan information.

*Token usage:* NetApp Console user token.


== Parameters

[cols=5*,options=header]
|===
|Name
|Type
|In
|Required
|Description

|partial
|boolean
|query
|False
a|Returns only a partial response from the NetApp Backup and Recovery service database. API calls with this query parameter skip fetching data from ONTAP.


|accountId
|string
|path
|True
a|Account ID


|workingEnvironmentId
|string
|path
|True
a|System ID


|authorization
|string
|header
|True
a|User token


|X-Agent-Id
|string
|header
|True
a|Request identifier


|===

== Response

```
Status: 200, Default response
```


[cols=4*,options=header]
|===
|Name
|Type
|Required
|Description

|volume
|array[link:#volume[volume]]
|False
a|

|===


.Example response
[%collapsible%closed]
====
[source,json,subs=+macros]
{
  "volume": [
    {
      "file-system-id": "string",
      "name": "string",
      "type": "string",
      "style": "string",
      "protocol": "string",
      "svm": "string",
      "status": "string",
      "backup-status": "string",
      "snapshot-count": "string",
      "backup-policy": {
        "name": "string",
        "rule": [
          {
            "label": "string",
            "retention": "string"
          }
        ],
        "archive-after-days": "string"
      },
      "snaplock-type": "string",
      "lag": "string",
      "capacity": {
        "allocated": "string",
        "used": "string",
        "charging": "string",
        "logicalUsedSize": "string"
      },
      "object-lock": "string",
      "ransomware-scan-status": "string",
      "ransomware-detected-snapshots": [
        {
          "snapshot-id": "string",
          "snapshot-name": "string"
        }
      ]
    }
  ]
}
====

== Definitions

[.api-def-first-level]
.See Definitions
[%collapsible%closed]
//Start collapsible Definitions block
====                
[#rule]
[.api-collapsible-fifth-title]
rule

[cols=4*,options=header]
|===
|Name
|Type
|Required
|Description

|label
|string
|False
a|

|retention
|string
|False
a|

|===


[#backup-policy]
[.api-collapsible-fifth-title]
backup-policy

[cols=4*,options=header]
|===
|Name
|Type
|Required
|Description

|name
|string
|False
a|

|rule
|array[link:#rule[rule]]
|False
a|

|archive-after-days
|string
|False
a|

|===


[#capacity]
[.api-collapsible-fifth-title]
capacity

[cols=4*,options=header]
|===
|Name
|Type
|Required
|Description

|allocated
|string
|False
a|

|used
|string
|False
a|

|charging
|string
|False
a|

|logicalUsedSize
|string
|False
a|

|===


[#ransomware-detected-snapshots]
[.api-collapsible-fifth-title]
ransomware-detected-snapshots

[cols=4*,options=header]
|===
|Name
|Type
|Required
|Description

|snapshot-id
|string
|False
a|

|snapshot-name
|string
|False
a|

|last-ransomware-scan
|number
|False
a|

|===


[#volume]
[.api-collapsible-fifth-title]
volume

[cols=4*,options=header]
|===
|Name
|Type
|Required
|Description

|file-system-id
|string
|False
a|

|name
|string
|False
a|

|type
|string
|False
a|

|style
|string
|False
a|

|protocol
|string
|False
a|

|svm
|string
|False
a|

|status
|string
|False
a|

|backup-status
|string
|False
a|

|last-backup-creation-time
|number
|False
a|

|snapshot-count
|string
|False
a|

|backup-policy
|link:#backup-policy[backup-policy]
|False
a|

|snaplock-type
|string
|False
a|

|lag
|string
|False
a|

|capacity
|link:#capacity[capacity]
|False
a|

|object-lock
|string
|False
a|

|ransomware-scan-status
|string
|False
a|

|last-ransomware-scan
|number
|False
a|

|ransomware-detected-snapshots
|array[link:#ransomware-detected-snapshots[ransomware-detected-snapshots]]
|False
a|

|geometry
|number
|False
a|

|aggregate-count
|number
|False
a|

|cbsChargingEnabled
|boolean
|False
a|

|===


//end collapsible .Definitions block
====
