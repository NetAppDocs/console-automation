...
* <<Add disks to aggregate for a single node, Single Node>>
* <<Add disks to aggregate for a high availability pair, HA pair>>

== Add disks to aggregate for a single node
You can use this workflow to add disks to an aggregate in a single node system.

=== 1. Select the system

Perform the workflow link:wf_gcp_cloud_get_wes.html[Get Google Cloud single node system] and choose the `publicId` value for the `workingEnvironmentId` path parameter.
...
== Add disks to aggregate for a high availability pair
You can use this workflow to add disks to an aggregate in an HA system.
