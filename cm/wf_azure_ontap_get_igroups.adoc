...
You can use this workflow to retrieve iGroups for a single node system.

=== 1. Select the system

Perform the workflow link:wf_azure_cloud_create_we_paygo.html#create-a-system-for-a-single-node[Create Azure single node system] and choose the `publicId` and `svmName` values for the system `workingEnvironmentId` and `svmName` path parameters.
...
You can use this workflow to retrieve iGroups for an HA system.

=== 1. Select the system

Perform the workflow link:wf_azure_cloud_get_wes.html#get-a-system-for-a-high-availability-pair[Create Azure single node system] and choose the `publicId` and `svmName` values for the system `workingEnvironmentId` and `svmName` path parameters.
