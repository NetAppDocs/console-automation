...
Perform the workflow link:wf_azure_ontap_get_volumes.html#get-volume-for-a-high-availability-pair[Get volumes] and choose the `name` and the `svmName` values of the volume to modify and use them for the `volumeName` and `svmName` path parameters.
