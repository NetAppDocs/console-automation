...
* <<Get tag keys for a single node, Single Node>>
* <<Get tag keys for a high availability pair, HA pair>>

== Get tag keys for a single node
You can use this workflow to retrieve the tag keys in a single node system.

=== Step 1. Select the project
Perform the workflow link:wf_gcp_cloud_md_get_projects.html#get-projects-for-a-single-node[Get projects] and choose `projectId` value of the required project for `projectId` query parameter.

=== Step 2. Select the region
Perform the link:wf_gcp_cloud_md_get_regions.html#get-regions-for-a-single-node[Get regions] workflow and choose the `zone: name` value of the required region for the `zone` query parameter.
...
== Get tag keys for a high availability pair
You can use this workflow to retrieve the tag keys in an HA system.

=== Step 1. Select the project
Perform the workflow link:wf_gcp_cloud_md_get_projects.html#get-projects-for-a-high-availability-pair[Get projects] and choose `projectId` value of the required project for `projectId` query parameter.

=== Step 2. Select the region
Perform the link:wf_gcp_cloud_md_get_regions.html#get-regions-for-a-high-availability-pair[Get regions] workflow and choose the `zone: name` value of the required region for the `zone` query parameter.
