...
Perform the workflow link:wf_azure_ontap_get_volumes.html#get-volumes-for-a-single-node[Get volumes] and choose the `name` and the `svmName` values of the volume to delete and use them for the `volumeName` and `svmName` path parameters.
...
Perform the workflow link:wf_azure_ontap_get_volumes.html#get-volumes-for-a-high-availability-pair[Get volumes] and choose the `name` and the `svmName` values of the volume to delete and use them for the `volumeName` and `svmName` path parameters.
