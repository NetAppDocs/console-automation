...
* If a CIFS configuration already exists, perform the workflow link:wf_aws_ontap_get_cifs.html#get-cifs-server-configuration-for-a-high-availability-pair[Get CIFS server configurations] to access the configuration parameters.
* If a CIFS configuration does not exist, perform the workflow link:wf_aws_ontap_create_cifs.html#create-cifs-server-configuration-for-a-high-availability-pair[Create CIFS server configuration] to create one.
...
Perform the workflow link:wf_aws_ontap_get_aggrs.html#get-aggregates-for-a-high-availability-pair[Get aggregates] and choose the `name` for the `aggregateName` value. From the `providerVolumes` field in the response, you must also choose the value of `diskType` for the `providerVolumeType` parameter, the value of `iops` for the `iops` parameter, and the value of `throughput` for the `throughput` parameter.
